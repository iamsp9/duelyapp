...
import type { CreditCard, ArchivedCard, PaymentStatus, BillingFrequency } from "@/types/card";
        ...
        keepHistory && card
          ? [
              ...state.vault.archivedCards,
              {
                ...
                totalBilled: (card.history ?? []).reduce(
      ...
      vault: {
        ...
        cards: state.vault.cards.map((card) => {
          ...
          return {
            ...
            // Record when it was disabled so we can preserve pre-disable bills
    ...
    set((state) => ({
      vault: {
        ...
        cards: state.vault.cards.map((card) => {
          ...
          // Track when the bill was set for this cycle (used for disabled card logic)
          if (updates.totalBill !== undefined && updates.totalBill !== '') {
            updatedCard.currentCycleBillSetAt = new Date().toISOString();
          }
