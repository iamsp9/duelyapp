...
export function CardItem({ card }: Props) {
  ...
  const billStr =
    card.totalBill !== "" && card.totalBill != null
      ? formatCurrency(card.totalBill)
      : "—";
  ...
  const renderStatusPreview = () => {
    const bill =
      parseFloat(String(billVal)) ||
      parseFloat(String(card.totalBill)) ||
      0;
    ...
    if (billVal !== "" && parseFloat(String(billVal)) === 0)
      return (
        <span className="text-green-400 text-[12px] flex items-center gap-1">
          ...
          No payment due — will be marked <b>Paid</b>
        </span>
      );
    ...
    if (proj > 0)
      return (
        <span className="text-orange-400 text-[12px] flex items-center gap-1">
          ...
          <b>Partial</b> — {formatCurrency(proj)} / {formatCurrency(bill)}
        </span>
      );
  ...
  const handleSave = (isConfirmed = false) => {
    ...
    updates.logOnly = `${new Date().toLocaleString("en-IN", {
      day: "numeric",
      month: "short",
      hour: "2-digit",
      minute: "2-digit",
    })}: bill set ${formatCurrency(billVal || 0)}`;
  ...
  const setOverride = (st: string) => {
    saveCardState(card.id, {
      statusOverride: st as import("@/types/card").PaymentStatus,
    });
  };
  ...
  return (
    <>
      <div
        className={`rounded-2xl border bg-[#111827] overflow-hidden transition-all ${glowClass}`}
      >
        <div
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[11px] text-slate-400 mt-[1px]">
              Bill {card.billDay}th · Due {card.dueDay}th
            </div>
          </div>

          <div
            className={`text-[11px] px-2 py-1 rounded-md font-semibold whitespace-nowrap shrink-0 ${badge.classes}`}
          >
            {badge.text}
          </div>
          ...
          <ChevronDown
            className={`size-[18px] text-slate-400 transition-transform shrink-0 ${open ? "rotate-180" : ""}`}
          />
        </div>
          ...
          <div className="p-3.5 border-t border-white/10">
            ...
            <div className="flex gap-1.5 mb-2.5">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-[10px] border text-[11px] font-semibold transition-all active:scale-95 min-h-[40px] ${status === "unpaid"
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-[10px] border text-[11px] font-semibold transition-all active:scale-95 min-h-[40px] ${status === "partial"
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-[10px] border text-[11px] font-semibold transition-all active:scale-95 min-h-[40px] ${status === "paid"
            ...
            <div className="flex flex-col gap-1 mb-2.5">
              ...
              <textarea
                ...
                className="bg-[#1a2234] border border-white/10 rounded-[10px] p-2.5 text-[15px] text-white focus:border-blue-500 outline-none min-h-[60px] w-full"
            ...
            <div className="mt-2.5 pt-2.5 border-t border-white/10">
              ...
              <div>
                {!(card.history && card.history.length) ? (
                  <div className="text-[12px] text-slate-500 py-1.5">
                    No payments yet
                  </div>
                ) : (
                  [...card.history].reverse().map((h, ri) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="py-1.5 border-b border-white/5"
                      >
                        <div className="flex items-center justify-between text-[12px] text-slate-300 gap-2">
                          <span className="flex-1 leading-[1.35]">
                            {h.amount
                              ? `📅 ${h.date || "—"} — ${formatCurrency(h.amount)}${h.note ? " · " + h.note : ""
                              }`
                              : h.text || "Log entry"}
                          </span>

                          <button
                            onClick={() => setDeleteConfirmId(idx)}
                        ...
                        {deleteConfirmId === idx && (
                          <div className="mt-2 rounded-xl border border-red-500/30 bg-red-500/10 p-3 flex items-center justify-between gap-3">
                            <div className="text-xs text-red-200">
                              Delete this payment history entry?
                            </div>
              ...
              {tp > 0 && (
                <div className="text-[13px] font-semibold text-white pt-2 mt-1 border-t border-white/10">
                  Total paid after bill generation: {formatCurrency(tp)}
                </div>
              )}
            </div>
          </div>
      ...
      {toast && (
        <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-[100]">
          <div
            className={`flex items-center gap-2 rounded-xl border px-4 py-3 shadow-2xl backdrop-blur-md text-sm font-medium transition-all ${toast.type === "success"
