  ...
  if (allClear) {
    return (
      <div className="space-y-4 animate-in fade-in duration-300">
        <QuickStatsStrip overdue={0} overdueTotal={0} dueSoon={0} dueSoonTotal={0} next30Total={0} />
      ...
      {overdue.length > 0 && (
        <section className="space-y-2">
          <SectionHeader
            ...
            total={overdueTotal} totalColor="text-red-400"
          />
      ...
      {dueSoon.length > 0 && (
        <section className="space-y-2">
          <SectionHeader
            ...
            total={dueSoonTotal} totalColor="text-orange-400"
          />
          <div className="flex flex-col gap-2">
...
function QuickStatsStrip({ overdue, overdueTotal, dueSoon, dueSoonTotal, next30Total }: {
  ...
  return (
    <div className="grid grid-cols-3 gap-2">
      ...
      <div className="rounded-2xl border border-white/10 bg-[#111827] p-3">
        ...
        <p className="text-[16px] font-bold leading-tight text-white">{fmt(next30Total)}</p>
...
function SectionHeader({ icon, label, count, total, totalColor }: {
  ...
  return (
    <div className="flex items-center justify-between px-1">
      ...
      {total > 0 && <p className={`text-[12px] font-semibold ${totalColor}`}>{formatCurrency(total)}</p>}
    </div>
  );
}
